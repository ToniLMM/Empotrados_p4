...
int read_quick_distance() {
  ...
  long duration = pulseIn(ECHO_PIN, HIGH, 19000);
...
void emergency_brake() {
  ...
  digitalWrite(PIN_Motor_AIN_1, LOW);
  digitalWrite(PIN_Motor_BIN_1, LOW);
  analogWrite(PIN_Motor_PWMA, 145); 
  analogWrite(PIN_Motor_PWMB, 145);
  delay(38);

  analogWrite(PIN_Motor_PWMA, 85);  
  analogWrite(PIN_Motor_PWMB, 85);
  delay(28); 
...
void handle_line_detection() {
  ...
  if (left_on_line && mid_on_line && right_on_line) {
    set_motors(BASE_SPEED + 28, BASE_SPEED + 28); 
    last_detection = 0;
    ...
    set_motors(BASE_SPEED - 10, BASE_SPEED + 15); 
    last_detection = 1;
  } else if (right_on_line && mid_on_line && !left_on_line) {
    set_motors(BASE_SPEED + 15, BASE_SPEED - 10);
    last_detection = 2;
  } else if (!left_on_line && !mid_on_line && !right_on_line) {
    if (last_detection == 1) {
      set_motors(-BASE_SPEED * 0.52, BASE_SPEED * 0.95); 
    } else if (last_detection == 2) {
      set_motors(BASE_SPEED * 0.95, -BASE_SPEED * 0.52);
    } else {
      set_motors(-BASE_SPEED / 3, BASE_SPEED / 3);
    }
  } else if (left_on_line && right_on_line && !mid_on_line) {
    set_motors(BASE_SPEED * 0.55, BASE_SPEED * 0.55); 
    last_detection = 0;
  }
}
